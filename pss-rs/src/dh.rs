...
pub use crypto_bigint::{modular::constant_mod::ResidueParams, ConcatMixed, Uint};

use crypto_bigint::{
    const_residue, generic_array::GenericArray, impl_modulus, modular::constant_mod::Residue,
    rand_core::OsRng, ArrayDecoding, CheckedSub, Encoding, NonZero, RandomMod, SplitMixed, U2048,
    U64,
};
...
pub const DH_MODP_2048_RESIDUE: Residue<DhModp2048Modulus, { U2048::LIMBS }> =
    const_residue!(DH_MODP_2048_GENERATOR, DhModp2048Modulus);
pub const DH_MODP_2048: PssDhPublicKey<DhModp2048Modulus, { U2048::LIMBS }> =
    PssDhPublicKey(DH_MODP_2048_RESIDUE);

const DH_TINY_TEST_INSECURE_MODULUS_HEX: &str = "000000000000001F";
impl_modulus!(
    DhTinyTestInsecureModulus,
    U64,
    DH_TINY_TEST_INSECURE_MODULUS_HEX
);
const DH_TINY_TEST_INSECURE_GENERATOR: U64 = U64::from_u8(3);
pub const DH_TINY_TEST: Residue<DhTinyTestInsecureModulus, { U64::LIMBS }> =
    const_residue!(DH_TINY_TEST_INSECURE_GENERATOR, DhTinyTestInsecureModulus);
...
impl<MOD: ResidueParams<LIMBS>, const LIMBS: usize> Into<Residue<MOD, LIMBS>>
    for PssDhPublicKey<MOD, LIMBS>
{
...
impl<MOD: ResidueParams<LIMBS>, const LIMBS: usize> TryFrom<Box<[u8]>>
    for PssDhPublicKey<MOD, LIMBS>
{
    type Error = ();
...
impl<MOD: ResidueParams<LIMBS>, const LIMBS: usize> Into<Box<[u8]>> for PssDhPublicKey<MOD, LIMBS>
where
    Uint<LIMBS>: Encoding,
{
    fn into(self) -> Box<[u8]> {
...
fn signature_hash<const LIMBS: usize, MOD: ResidueParams<LIMBS>, T: AsRef<[u8]>>(
    ...
    a1_i_sector_icc_1: Option<(PssDhPublicKey<MOD, LIMBS>, &PssDhPublicKey<MOD, LIMBS>)>,
    a2_i_sector_icc_2: Option<(PssDhPublicKey<MOD, LIMBS>, &PssDhPublicKey<MOD, LIMBS>)>,
    pk_sector: &PkSector<LIMBS, MOD>,
    message: &[u8],
) -> GenericArray<u8, <sha3::Keccak256 as OutputSizeUser>::OutputSize>
where
    Uint<LIMBS>: Encoding<Repr = T>,
{
    ...
    Uint<LIMBS>: Encoding,
{
    ...
    s2: Uint<LIMBS>,
    pseudonyms: (
        Option<PssDhPublicKey<MOD, LIMBS>>,
        Option<PssDhPublicKey<MOD, LIMBS>>,
    ),
}
...
impl<const LIMBS: usize, MOD: ResidueParams<LIMBS>> TryFrom<GenericPssSignature>
    for GroupPssSignature<LIMBS, MOD>
where
    Uint<LIMBS>: Encoding,
{
    type Error = ();
    ...
    fn try_from(value: GenericPssSignature) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            pseudonyms: (
                value
                    .pseudonym1
                    .map(|p| Residue::new(&Uint::from_be_slice(&p)).into()),
                value
                    .pseudonym2
                    .map(|p| Residue::new(&Uint::from_be_slice(&p)).into()),
            ),
...
impl<const LIMBS: usize, MOD: ResidueParams<LIMBS>> From<GroupPssSignature<LIMBS, MOD>>
    for GenericPssSignature
where
    Uint<LIMBS>: Encoding,
{
    fn from(value: GroupPssSignature<LIMBS, MOD>) -> Self {
        GenericPssSignature {
            ...
            pseudonym1: value
                .pseudonyms
                .0
                .map(|p| p.retrieve().to_be_bytes().as_ref().into()),
            pseudonym2: value
                .pseudonyms
                .1
                .map(|p| p.retrieve().to_be_bytes().as_ref().into()),
...
impl<'a, const LIMBS: usize, const WIDE_LIMBS: usize, MOD: ResidueParams<LIMBS>> PssSigner
    for GroupPssSigner<'a, LIMBS, MOD>
where
    Uint<LIMBS>: Encoding + ConcatMixed<MixedOutput = Uint<WIDE_LIMBS>>,
    Uint<WIDE_LIMBS>: SplitMixed<Uint<LIMBS>, Uint<LIMBS>>,
{
    type PssSignature = GroupPssSignature<LIMBS, MOD>;

    fn sign(&self, message: &[u8]) -> GroupPssSignature<LIMBS, MOD> {
        let k1 =
            Uint::<LIMBS>::random_mod(&mut OsRng::default(), &NonZero::from_uint(MOD::MODULUS));
        let k2 =
            Uint::<LIMBS>::random_mod(&mut OsRng::default(), &NonZero::from_uint(MOD::MODULUS));
...
impl<const LIMBS: usize, const WIDE_LIMBS: usize, MOD: ResidueParams<LIMBS>> Icc
    for GroupIcc<LIMBS, WIDE_LIMBS, MOD>
where
    Uint<LIMBS>: Encoding + ConcatMixed<MixedOutput = Uint<WIDE_LIMBS>>,
    Uint<WIDE_LIMBS>: SplitMixed<Uint<LIMBS>, Uint<LIMBS>>,
{
    ...
    fn sector_identifiers(
        &self,
        pk_sector: &PkSector<LIMBS, MOD>,
    ) -> GroupSectorSpecificIdentifiers<LIMBS, MOD> {
    ...
    fn signer<'a>(
        &'a self,
        pk_sector: &'a PkSector<LIMBS, MOD>,
        use_identifier1: bool,
        use_identifier2: bool,
    ) -> GroupPssSigner<'a, LIMBS, MOD> {
...
impl<const LIMBS: usize, const WIDE_LIMBS: usize, MOD: ResidueParams<LIMBS>>
    From<GroupIcc<LIMBS, WIDE_LIMBS, MOD>> for GenericIccSecretKey
where
    Uint<LIMBS>: Encoding + ConcatMixed<MixedOutput = Uint<WIDE_LIMBS>>,
    Uint<WIDE_LIMBS>: SplitMixed<Uint<LIMBS>, Uint<LIMBS>>,
{
...
impl<const LIMBS: usize, MOD: ResidueParams<LIMBS>> GroupManagerPublicKey
    for GroupGroupManagerPublicKey<LIMBS, MOD>
where
    Uint<LIMBS>: Encoding,
{
    type PublicKey = PssDhPublicKey<MOD, LIMBS>;
    ...
    fn new(
        pk_icc: PssDhPublicKey<MOD, LIMBS>,
        pk_m: PssDhPublicKey<MOD, LIMBS>,
        g: Option<PssDhPublicKey<MOD, LIMBS>>,
    ) -> Self {
        Self {
            g: g.unwrap(),
            pk_icc,
    ...
    fn check_signature(
        ...
        message: &[u8],
        pk_sector: &PkSector<LIMBS, MOD>,
        signature: &GroupPssSignature<LIMBS, MOD>,
    ) -> bool {
...
impl<const LIMBS: usize, MOD: ResidueParams<LIMBS>> From<GroupGroupManagerPublicKey<LIMBS, MOD>>
    for GenericGroupManagerPublicKey
where
    Uint<LIMBS>: Encoding,
{
...
impl<const LIMBS: usize, const WIDE_LIMBS: usize, MOD: ResidueParams<LIMBS>> GroupManager
    for GroupGroupManager<LIMBS, WIDE_LIMBS, MOD>
where
    Uint<LIMBS>: Encoding + ConcatMixed<MixedOutput = Uint<WIDE_LIMBS>>,
    Uint<WIDE_LIMBS>: SplitMixed<Uint<LIMBS>, Uint<LIMBS>>,
{
...
impl<const LIMBS: usize, const WIDE_LIMBS: usize, MOD: ResidueParams<LIMBS>>
    From<GroupGroupManager<LIMBS, WIDE_LIMBS, MOD>> for GenericGroupManagerPrivateKey
where
    Uint<LIMBS>: Encoding + ConcatMixed<MixedOutput = Uint<WIDE_LIMBS>>,
    Uint<WIDE_LIMBS>: SplitMixed<Uint<LIMBS>, Uint<LIMBS>>,
{
...
mod tests {
    ...
    fn invalid_signature() {
        ...
        for (id1, id2) in combinations {
            ...
            let signature = signer.sign(SIGN_MESSAGE);
            fn tamper_with<const LIMBS: usize, MOD: ResidueParams<LIMBS>>(
                signature: &mut GroupPssSignature<LIMBS, MOD>,
                c: bool,
                s1: bool,
                s2: bool,
            ) where
